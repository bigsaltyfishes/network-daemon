...
impl From<Ipv4Addr> for sockaddr_in {
    fn from(ip: Ipv4Addr) -> Self {
        ...
        sockaddr_in {
            ...
            // `sin_zero` is `[c_char; 8]`: c_char is i8 on FreeBSD amd64 but
            // u8 on FreeBSD aarch64. Use a typed `[0; 8]` so the literal
            // matches c_char on every target (upstream hardcoded `[0i8; 8]`,
            // which breaks aarch64). See vendored-crate note in Cargo.toml.
            sin_zero: [0; 8],
        }
    }
}
